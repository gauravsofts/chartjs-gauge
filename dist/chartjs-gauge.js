/*!
 * chartjs-gauge.js v0.3.1
 * https://github.com/gauravsofts/chartjs-gauge
  ...
  */
  drawNeedle: function drawNeedle(ease) {
    ...
    var _this$chart2 = this.chart,
        ...
        innerRadius = _this$chart2.innerRadius,
        outerRadius = _this$chart2.outerRadius;
    var dataset = config.data.datasets[this.index];

    var _this$getMeta2 = this.getMeta(),
        previous = _this$getMeta2.previous;

    var _config$options$needl2 = config.options.needle,
        radiusPercentage = _config$options$needl2.radiusPercentage,
        widthPercentage = _config$options$needl2.widthPercentage,
        lengthPercentage = _config$options$needl2.lengthPercentage,
        color = _config$options$needl2.color;
    ...
    var _this$getTranslation2 = this.getTranslation(this.chart),
        ...
        dy = _this$getTranslation2.dy; // interpolate
  ...
  drawMarker: function drawMarker(ease) {
    if (!this.chart.animating) {
      // triggered when hovering
      ease = 1;
    }
    ...
    var width = this.getWidth(this.chart);
    var needleWidth = 0;
    var needleLength = outerRadius - innerRadius;
    ...
    var target = this.getAngle({
      chart: this.chart,
      valuePercent: this.getValuePercent(dataset, dataset.marker)
    });
    var angle = origin + (target - origin) * ease; // draw
    ...
    ctx.beginPath();
    ctx.setLineDash([2, 5]);
    ctx.moveTo(innerRadius, needleWidth / 2);
    ctx.lineTo(innerRadius + needleLength, 0);
    ctx.lineTo(innerRadius, -needleWidth / 2);
    ctx.stroke();
    ctx.restore();
  },
